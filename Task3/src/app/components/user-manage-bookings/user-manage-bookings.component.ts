...
export class UserManageBookingsComponent {
    ...
    upcoming_bookings: Observable<Booking[]> = this.bookings_service.bookingsData.pipe(
        map((bookings) => bookings.filter((b) => new Date(b.flight.boarding_date) >= new Date()))
    );

    past_bookings: Observable<Booking[]> = this.bookings_service.bookingsData.pipe(
        map((bookings) => bookings.filter((b) => new Date(b.flight.boarding_date) < new Date()))
    );
}
